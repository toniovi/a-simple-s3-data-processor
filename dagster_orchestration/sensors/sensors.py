...
from dagster import (sensor, SensorResult, SensorEvaluationContext,
                    RunRequest, AssetKey)
...
def check_for_new_s3_files(context: SensorEvaluationContext,
    ...
    for result in paginator.paginate(Bucket=bucket):
        for file in result.get('Contents', []):
            file_name = file['Key']
            if not context.instance.has_dynamic_partition(files_partitions_def.name, file_name):
                new_files.append(file_name)

    # Return DynamicPartitionsDefinition to generate new partitions for new files
    return SensorResult(
        run_requests=[RunRequest(partition_key=file_name) for file_name in new_files],
        dynamic_partitions_requests=[files_partitions_def.build_add_request(new_files)],
    )
